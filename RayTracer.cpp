...
LightIntensity RayTracer::TraceRayStream(const Ray&ray, Scene*scene,
    ...
    MaterialType materialType = closestIntersection.object->GetMaterial()->type;

    if(materialType==REFLECTIVE && reflections>0) {
        Vector3 reflected = ray.direction.Reflect(closestIntersection.intersectionLPOINTNormal);
        reflected.Normalize();
        Ray newRay(closestIntersection.LPOINT+reflected*BIAS, reflected);
    ...
    else if(materialType==REFRACTIVE && reflections>0) {
        float reflectionCoef = max(0.0, min(1.0, 0.05+0.11*(pow(1+ray.direction.DotProduct(closestIntersection.intersectionLPOINTNormal), 1))));
        LightIntensity reflectedIntensity;
        LightIntensity refractedIntensity;
        ...
        if(globalMap) {
            QList<Stream*> closest = globalMap->GetClosestPhotons(closestIntersection.LPOINT, globalRadius, globalNumPhotons);
        ...
        if(causticMap) {
            QList<Stream*> closest = causticMap->GetClosestPhotons(closestIntersection.LPOINT, causticRadius, causticNumPhotons);
