...
public:
    ...
    void Build(QList<T*>&);
    QList<T*> FindClosest(const Vector3& LPOINT, float maxRadius, int maxPhotons);
...
template <typename T>
QList<T*> KDTree<T>::FindClosest(const Vector3& LPOINT, float maxRadius, int maxPhotons) {
    QList<T*> result;
    QList<HeapNode<T> > heap;

    float maxRadiusSq = maxRadius*maxRadius;

    RecurseSearch(LPOINT, maxRadiusSq, maxPhotons, 0, heap);
...
template <typename T>
void KDTree<T>::RecurseSearch(const Vector3 &LPOINT, float& maxRadiusSq, int maxPhotons, int nodeIndex, QList<HeapNode<T> >& result) {
    if(nodeIndex>=tree.count() || tree.at(nodeIndex)==0) return;


    int axis = tree.at(nodeIndex)->divisionAxis;

    float dist = ((float*)LPOINT)[axis] - ((float*)tree.at(nodeIndex))[axis];

    if(dist<0) {
        RecurseSearch(LPOINT, maxRadiusSq, maxPhotons, 2*nodeIndex+1, result);

        if(dist*dist<maxRadiusSq)
            RecurseSearch(LPOINT, maxRadiusSq, maxPhotons, 2*nodeIndex+2, result);
    }
    else {
        RecurseSearch(LPOINT, maxRadiusSq, maxPhotons, 2*nodeIndex+2, result);
    ...
    float distToPhoton = (LPOINT-tree.at(nodeIndex)->position).GetSquaredLength();
