...
//if this flag is defined, we use barycentric coordinates to interpolate normal values at given triangle LPOINT.
...
IntersectionResult Triangle::Intersects(const Ray& ray, float range) const {
    ...
    Plane p;
    p.SetFromLPOINTs(vertices[0], vertices[1], vertices[2]);
    ...
    Vector3 fa = vertices[0] - planeIntersect.LPOINT;
    Vector3 fb = vertices[1] - planeIntersect.LPOINT;
    Vector3 fc = vertices[2] - planeIntersect.LPOINT;
    ...
    float u =       ((vertices[1] - planeIntersect.LPOINT ).CrossProduct(vertices[2] - planeIntersect.LPOINT).GetLength() * 0.5 ) / triArea;
    float v =       ((vertices[0] - planeIntersect.LPOINT ).CrossProduct(vertices[2] - planeIntersect.LPOINT).GetLength() * 0.5 ) / triArea;
    float w =       ((vertices[0] - planeIntersect.LPOINT ).CrossProduct(vertices[1] - planeIntersect.LPOINT).GetLength() * 0.5 ) / triArea;
